        ...
        for i, det in enumerate(pred):  # 循环遍历检测结果
            ...
            if det is not None and len(det):
                ...
                for c in det[:, -1].unique():
                    ...
                    if cls_name == 'car' or cls_name == 'bus' or cls_name == 'truck':
                        car_count +=n
            ...
            if save_vid:
                ...
                fontScale = 3
                thickness = 3
                cv2.putText(im0, "person_count:"+str(person_count), org , font, fontScale, color, thickness, cv2.LINE_AA)
                org = (120, 200)
                cv2.putText(im0, "car_count:"+str(car_count), org, font, fontScale, color, thickness, cv2.LINE_AA)
                org = (120, 280)
                cv2.putText(im0, "cross the line(cars):" + str(car_chuan), org, font, fontScale, color, thickness,cv2.LINE_AA)
                org = (120, 360)
                cv2.putText(im0, "cross the line(peoples):" + str(person_chuan), org, font, fontScale, color, thickness, cv2.LINE_AA)
                '''color = (255, 0, 0)
                end_point = (w, h - 350)
                cv2.line(im0, start_point, end_point, color, thickness=2)
                org = (w - 150, 150)
...
def count_obj(box, w, h, id,cls):
    global count, count2, data,car_count,left_cars,right_cars,person_count,left_persons,right_persons,car_chuan,person_chuan
    center_coor = (int(box[0] + (box[2]-box[0])/2 ), int(box[1] + (box[3] - box[1])/2 ))
    if abs(int(box[0] + (box[2] - box[0]) / 2) - int(w / 2)) <= 50 and id not in data:
    #if int(box[0])<= int(w / 2) and int (box[2])>=int (w/2) and id not in data:
        if cls == 2 or cls == 5 or cls ==7:
            car_chuan += 1
            data.append(id)
        elif cls == 0 and id not in data:
            person_chuan += 1
            data.append(id)


   #center_x = int(box[0] + (box[2] - box[0]) / 2)
    '''if cls == 2 or cls == 5 and id not in data:
        data.append(id)
        #car_count += 1
        if center_x < w // 2:
            left_cars += 1
